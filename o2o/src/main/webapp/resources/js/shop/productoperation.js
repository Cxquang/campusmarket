$(function(){
	...
	function getInfo(id){
		$.getJSON(infoUrl,function(data){
			if(data.success){
				//返回的JSON当中获取product对象的信息，并赋值给表单
				...
				optionArr.map(function(item,index){
					...
					+ isSelect + '>' + item.productCategoryName + '</option>';
				});
	...
	//针对商品详情图控件组，若该控件组的最后一个元素发生变化(即上传了图片),且控件总数未操作6个，则生成新的一个文件上传控件
		...
		$.ajax({
			...
			success: function(data){
				if(data.success){
					...
					$('#captcha_img').click();$('#captcha_img').click();
				}
			}
		});
	});
